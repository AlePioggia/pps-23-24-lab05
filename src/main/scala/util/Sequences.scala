...
object Sequences: // Essentially, generic linkedlists
  ...
  object Sequence:
    def apply[A](elements: A*): Sequence[A] =
      ...
      for e <- elements do sequence = Cons(e, sequence)
      sequence.reverse()
    ...
    extension [A](sequence: Sequence[A])
      ...
      def map[B](f: A => B): Sequence[B] =
        sequence.flatMap(x => Cons(f(x), Nil()))
      ...
      def foldLeft[B](initial: B)(f: (B, A) => B): B = sequence match
        case Cons(h, t) => t.foldLeft(f(initial, h))(f)
        case _          => initial

      def intersect(other: Sequence[A]): Sequence[A] =
        sequence.filter(other.contains)
