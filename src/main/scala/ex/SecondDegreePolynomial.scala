...
@main def checkComplex(): Unit =
  ...
  println(
    (sum, sum.secondDegree, sum.firstDegree, sum.constant)
  ) // 1.0 * X^2 + 1.0 * X + 3.0
  val multipleOperations =
    fullPolynomial - (anotherPolynomial + simplePolynomial)
  println(
    (
      multipleOperations,
...
/** Hints:
  *   - implement SecondDegreePolynomial with a SecondDegreePolynomialImpl
  *     class, similar to PersonImpl in slides
